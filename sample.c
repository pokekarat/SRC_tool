...
int main(int argc, char **argv)
{
    if(argc < 5)
    {
        printf("Please specify [delay time (sec), number of loops, packagename, wifi-path], e.g., 1 100 com.snctln.game.BreakTheBlocks /sys/class/net/wlan0/ \n"); 
        exit(-1);
    }
    ...
    if(argc==5)
    {
	    ...
	    strcpy(wifi_path, argv[4]);
    }
    
    char aut[100];
    char freqs[100];
    char wifi_path2[1024];
    int ncolumns 		= 	size;
    int isKilled 		= 	1;
    ...
    for(;;)
    {
        if(pidLen == 0)
        {
            sprintf(aut, "ps | grep %s > /dev/null", filename);
            if(system(aut) == 0)
            {
                sprintf(aut,"ps | grep %s >> /data/local/tmp/tmp_aut", filename);
                system(aut);
	...
	/// Wi-Fi tx packets
        sprintf(wifi_path2,"%s/statistics/tx_packets",wifi_path);
  	if((fp_tx_pk = fopen(wifi_path2,"r")) != NULL)
        {
	...
	/// Wi-Fi rx packets
	sprintf(wifi_path2,"%s/statistics/rx_packets",wifi_path);
  	if((fp_rx_pk = fopen(wifi_path2,"r")) != NULL)
        {
        ...
        /// Wi-Fi tx bytes
        sprintf(wifi_path2,"%s/statistics/tx_bytes",wifi_path);
  	if((fp_tx_byte = fopen(wifi_path2,"r")) != NULL)
        {
	...
	/// Wi-Fi rx bytes
	sprintf(wifi_path2,"%s/statistics/rx_bytes",wifi_path);
  	if((fp_rx_byte = fopen(wifi_path2,"r")) != NULL)
        {
